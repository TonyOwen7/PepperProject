...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        commands = [
            (0, 'Lancer DCM', 'Lance le DCM pour connecter Pepper via ROS avec IP.', 'roslaunch pepper_dcm_bringup pepper_bringup.launch robot_ip:=192.168.1.34 roscore_ip:=localhost network_interface:=enp0s3'),
            (1, 'Driver Naoqi', 'Démarre le driver Naoqi pour interagir avec Pepper.', 'roslaunch naoqi_driver naoqi_driver.launch nao_ip:=192.168.0.126 roscore_ip:=localhost network_interface:=enp0s8'),
            (2, 'Calibrer caméra', 'Lance l’outil de calibration de la caméra.', 'rosrun camera_calibration cameracalibrator.py --size 8x6 --square 0.108 image:=/cv_camera/image_raw camera:=/cv_camera'),
            (3, 'Contrôle joystick', 'Permet de contrôler Pepper via une manette (joystick).', 'roslaunch teleop_twist_joy pepper.launch'),
            (4, 'stop', 'Arrête immédiatement tous les mouvements de Pepper.', 'rostopic pub /cmd_vel geometry_msgs/Twist "[0, 0, 0]" "[0, 0, 0]"'),
            ...
            (7, 'Aller à gauche', 'Déplace Pepper latéralement vers la gauche.', 'rostopic pub /cmd_vel geometry_msgs/Twist "[0, 0.5, 0]" "[0, 0, 0]"'),
            (8, 'Aller à droite', 'Déplace Pepper latéralement vers la droite.', 'rostopic pub /cmd_vel geometry_msgs/Twist "[0, -0.5, 0]" "[0, 0, 0]"'),
            (9, 'Avancer vers la gauche', 'Fait avancer Pepper en diagonale vers la gauche.', 'rostopic pub /cmd_vel geometry_msgs/Twist "[0.5, 0.5, 0]" "[0, 0, 0]"'),
            (10, 'Avancer vers la droite', 'Fait avancer Pepper en diagonale vers la droite.', 'rostopic pub /cmd_vel geometry_msgs/Twist "[0.5, -0.5, 0]" "[0, 0, 0]"'),
            (11, 'Reculer vers la gauche', 'Fait reculer Pepper en diagonale vers la gauche.', 'rostopic pub /cmd_vel geometry_msgs/Twist "[-0.5, 0.5, 0]" "[0, 0, 0]"'),
            (12, 'Reculer vers la droite', 'Fait reculer Pepper en diagonale vers la droite.', 'rostopic pub /cmd_vel geometry_msgs/Twist "[-0.5, -0.5, 0]" "[0, 0, 0]"'),
